...
<form name="form1" method="post" action="item_mermas.php">
      ...
      <tr>
        ...
        <td width="425">
		<? $mermas = mysqli_query($link,"Select tipoMerma, nombreMerma From tipoMermas WHERE tipoNota = $tipo_nota");?>
        ...
        <input name="id_merma" type="hidden" id="id_merma" value="<? echo $id_merma?>">
		<input name='tn' type='hidden' value='<? echo $tipo_nota?>'>
	</td>
      ...
      <tr>
        ...
        <td><?
		if($tipo_nota == 3){
			$prod = mysqli_query($link,'SELECT IdProducto, NombreProducto, TipoProducto FROM productos WHERE tipoProducto BETWEEN 1 AND 6');
		}
		else{
			$prod = mysqli_query($link,'SELECT IdProducto, NombreProducto, TipoProducto FROM productos WHERE tipoProducto BETWEEN 5 AND 6');
		}
	?>
